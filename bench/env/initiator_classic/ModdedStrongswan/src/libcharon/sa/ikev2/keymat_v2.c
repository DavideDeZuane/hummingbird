	...
	private_keymat_v2_t *this, key_exchange_method_t method)
{
	...
	key_exchange_t* gen =  lib->crypto->create_ke(lib->crypto, method);
    ...
    DBG1(DBG_IKE, "keymatv2.c - Elapsed CPU time for generating key: %.6f seconds", elapsed);
	return gen;
}
	...
	private_keymat_v2_t *this)
{
	...
	struct timespec start, end;
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
	nonce_gen_t *gen = lib->crypto->create_nonce_gen(lib->crypto);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);

    double elapsed = (end.tv_sec - start.tv_sec) + 
                     (end.tv_nsec - start.tv_nsec) / 1e9;

    DBG1(DBG_IKE, "keymatv2.c - Elapsed CPU time for generating nonce: %.6f seconds", elapsed);
	return gen;
}
	...
	pseudo_random_function_t rekey_function, chunk_t rekey_skd)
{
	...
	*((uint64_t*)spi_r.ptr) = id->get_responder_spi(id);
    ...
    DBG1(DBG_IKE, "keymatv2.c - Elapsed CPU time for generating keymat: %.6f seconds", elapsed);
failure:
	chunk_clear(&keymat);
	DESTROY_IF(prf_plus);
	return success;
